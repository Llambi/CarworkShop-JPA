...
public class Printer {
    ...
    public static void printPaymentMeans(
	    List<PaymentMeanDto> medios) {
	Console.println();
	Console.println("Medios de pago disponibles");

	Console.printf("\t%s \t%-8.8s \t%s %n", "ID", "Tipo",
		"Acumulado");
	...
	Console.printf("\t%s \t%-8.8s \t%s %n", medio.id,
	...
	Console.printf(
		"\t%d \t%-40.40s \t%td/%<tm/%<tY \t%-12.12s \t%.2f%n",
		rep.id, rep.description, rep.date, rep.status,
		rep.total);
    }

    public static void printMechanic(MechanicDto m) {

	Console.printf("\t%d %-10.10s %-25.25s %-25.25s%n", m.id,
		m.dni, m.name, m.surname);
    }

    public static void printContractCategory(ContractCategoryDto c) {

	Console.printf("\t%d %-20.20s %7.2f %3.2f%n", c.id, c.name,
		c.trieniumSalary, c.productivityPlus);

    }

    public static void printContractType(ContractTypeDto t) {

	Console.printf("\t%d %-20.20s %2d%n", t.id, t.name,
		t.compensationDays);
	...
	Console.printf(
		...
		c.id, c.dni, c.categoryName, c.typeName, c.status,
		c.startDate, c.startDate, c.startDate,
		c.yearBaseSalary, c.compensation, c.endDate,
		c.endDate, c.endDate);
	...
	Console.printf("\t%d %tm/%tY %7.2f%n", p.id, p.date, p.date,
		p.netTotal);
	...
	Console.printf("\t%d %tm/%tY %7.2f%n %7.2f%n %7.2f%n", p.id,
		p.date, p.date, p.grossTotal, p.discountTotal,
		p.netTotal);
